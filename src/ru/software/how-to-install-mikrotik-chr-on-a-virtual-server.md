...
2. В данном примере мы будем использовать:
   - Тариф: DE-BUDGET-1
   - Базовая ОС: Ubuntu 26.04 (необходима для развертывания CHR)

::: warning
Для установки CHR подходят не все операционные системы. Наши тесты показали, что установка работает только в Debian 12, Debian 13 и Ubuntu 26.04. Установка CHR в другой ОС может привести к ошибкам во время запуска или работы. Если вы все же хотите установить CHR в ОС, которой нет в списке, рекомендуем попробовать более старую версию, в частности `6.49.20`, или установить CHR в режиме восстановления.
:::

## Установка из работающей ОС

::: warning
Этот способ не рекомендуется. Мы настоятельно рекомендуем выполнять установку в режиме восстановления.
:::

### 1. Подготовка сервера

- Сервер будет готов в течение 2 минут после заказа
- Подключитесь к серверу, используя учетные данные SSH из письма об активации или переустановке ОС
- Инструкции по подключению через SSH приведены в руководстве [Как подключиться к серверу (Linux)](/vps/how-to-connect-through-ssh)
...
Обновите базу данных пакетов и установите `unzip`:
...
Оставшаяся настройка будет выполняться через веб-интерфейс VNC. Дальнейшие действия приведены в разделе [Настройка сети](#настройка-сети).
...
- Сервер будет готов в течение 2 минут после заказа
- Войдите в свою учетную запись в [панели управления VM](https://vm.senko.digital/)
- Переведите сервер в режим восстановления, воспользовавшись нашим [руководством](/vps/working-with-rescue)

### 2. Загрузка образа CHR

- Перейдите на [официальную страницу загрузки MikroTik](https://mikrotik.com/download/chr)
- Загрузите версию CHR **Raw disk image** (в этом примере используется версия 7.20.8)
![Изображение, показывающее, какую версию MikroTik CHR следует использовать для виртуального сервера](/images/vps/mikrotik-chr/select-mikrotik-chr-version.png){data-zoomable}
- Нажмите правой кнопкой мыши на значок загрузки и скопируйте ссылку (например, https://download.mikrotik.com/routeros/7.20.8/chr-7.20.8.img.zip)
- Загрузите образ на виртуальный сервер:
...
![Изображение, показывающее успешную загрузку образа MikroTik CHR на виртуальный сервер в режиме восстановления](/images/vps/mikrotik-chr/successful-download-mikrotik-chr-rescue.png){data-zoomable}

### 3. Развертывание образа

::: warning
Этот процесс **удалит все данные с сервера**.
Перед продолжением обязательно **создайте резервную копию важных данных**!
:::
...
После завершения последней команды отключите ISO и **установите флажок "ОС переустановлена"**.
