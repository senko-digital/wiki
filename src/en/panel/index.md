...
- **[Backups](./backups)** - server data backups, can be set up with the scheduler for regular backups
- **[Scheduler](./schedules)** - allows you to automate many functions, such as inputting commands or backing up the server at a specific or regular time
- **[Users](./users)** - adding new users, flexible configuration of their available permissions
- **[Startup Parameters](./startup)** - managing parameters that will be used each time the server starts
- **[Settings](./settings)** - information for connecting to the server via SFTP, changing its name in the panel, and more
